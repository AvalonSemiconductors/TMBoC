...
data_extract_v1_d #(.N(N),.es(es)) uut_de1(.in(xin1), .rc(rc1), .regime(regime1), .exp(e1), .mant(mant1));
data_extract_v1_d #(.N(N),.es(es)) uut_de2(.in(xin2), .rc(rc2), .regime(regime2), .exp(e2), .mant(mant2));
...
generate 
	...
	if (NR_Iter > 0) begin
		...
		for (i = 0; i < NR_Iter; i=i+1)begin : NR_Iteration
			...
			sub_N_d #(.N(M+1)) uut_sub_m2 ({1'b1,{M{1'b0}}}, {1'b0,m2_inv_X_m2[i][2*M+1:M+3],|m2_inv_X_m2[i][M+2:0]}, two_m2_inv_X_m2[i]);
...
wire [es-1:0] e_o;
wire [Bs:0] r_o;
reg_exp_op_d #(.es(es), .Bs(Bs)) uut_reg_ro (div_e[es+Bs+1:0], e_o, r_o);
...
DSR_right_N_S_d #(.N(3*N+3), .S(Bs+1)) dsr2 (.a({tmp_o,{N{1'b0}}}), .b(r_o[Bs] ? {Bs{1'b1}} : r_o), .c(tmp1_o));
...
add_N_d #(.N(N)) uut_add_ulp (tmp1_o[2*N-1+3:N+3], rnd_ulp, tmp1_o_rnd_ulp);
...
/////////////////////////
module data_extract_v1_d(in, rc, regime, exp, mant);
function [31:0] log2;
input reg [31:0] value;
	begin
	value = value-1;
...
wire [Bs-1:0] k;
LOD_N_d #(.N(N)) xinst_k(.in({xin_r[N-2:0],rc^1'b0}), .out(k));

assign regime = rc ? k-1 : k;

wire [N-1:0] xin_tmp;
DSR_left_N_S_d #(.N(N), .S(Bs)) ls (.a({xin[N-3:0],2'b0}),.b(k),.c(xin_tmp));
...
/////////////////////////
module reg_exp_op_d (exp_o, e_o, r_o);
parameter es=3;
parameter Bs=5;
...
conv_2c_d #(.N(es+Bs)) uut_conv_2c1 (~exp_o[es+Bs:0],exp_oN_tmp);
...
wire vld;
LOD_d #(.N(N)) l1 (in, out, vld);
endmodule


/////////////////////////
module LOD_d (in, out, vld);
